 ...
 * GET /api/ai/cici
 */
export const ciciAI = async (req, res) => {
  try {
    ...
    // Build URL
    const externalUrl = `https://anabot.my.id/api/ai/cici?prompt=${encodeURIComponent(prompt)}&apikey=${encodeURIComponent(finalApikey)}`;

    // Fetch data
    const result = await get(externalUrl);

    if (!result.success) {
      return errorResponse(res, 'Failed to get Cici AI response', result.error);
    }

    return successResponse(res, result.data, 'Cici AI response generated successfully');
  } catch (error) {
